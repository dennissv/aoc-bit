...
class SimpleNavigator():
    ...
    def _calculate_distances(self):
        ...
        self.distances = {char: {} for char in ascii_lowercase+ascii_uppercase+'@'}
    ...
    def _build_order(self):
        ...
        for target in ascii_lowercase+ascii_uppercase:
            ...
            if target not in self.positions:
                continue
            for node in nx.astar_path(self.g, self.positions['@'], self.positions[target]):
                y, x = node
                char = self.grid[y][x]
    ...
    def move(self, target):
        ...
        self.distance_traveled += self.distances[self.history[-1]][target]
